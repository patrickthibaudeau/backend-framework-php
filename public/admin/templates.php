...
global $DB; // ensure global DB is used
...
if ($action==='update_template_caps' && $templateId) {
    ...
    foreach ($caps as $c) {
        ...
        if ($perm==='') {
            if ($existing) {
                $DB->delete_records('role_template_capabilities',[ 'templateid'=>$templateId,'capability'=>$c->name ]);
                $AM->logAction($currentUser->getId(),'template_capability_remove',['cap'=>$c->name],null,null,$c->name);
            }
        } else {
            if ($existing) {
                if ($existing->permission !== $perm) {
                    $DB->update_record('role_template_capabilities',[ 'id'=>$existing->id, 'permission'=>$perm, 'timemodified'=>$now ]);
                    $AM->logAction($currentUser->getId(),'template_capability_set',['cap'=>$c->name,'perm'=>$perm],null,null,$c->name);
                }
            } else {
                $DB->insert_record('role_template_capabilities',[ 'templateid'=>$templateId,'capability'=>$c->name,'permission'=>$perm,'timecreated'=>$now,'timemodified'=>$now ]);
                $AM->logAction($currentUser->getId(),'template_capability_set',['cap'=>$c->name,'perm'=>$perm],null,null,$c->name);
            }
        }
    }
...
if ($action==='delete_template' && $templateId) {
    ...
    $template = $DB->get_record('role_templates',['id'=>$templateId]);
    if ($template) {
        $DB->delete_records('role_templates',[ 'id'=>$templateId ]);
...
if ($editingTemplate) {
    ...
    $capView = array_map(function($c) use ($tMap){
        $perm=$tMap[$c->name]??'';
        $options = array_merge([
            ['value'=>'','label'=>'inherit-none','selected'=>$perm==='']
        ], array_map(function($p) use ($perm){ return ['value'=>$p,'label'=>$p,'selected'=>$p===$perm]; }, ['allow','prevent','prohibit']));
        return [
            'name'=>$c->name,
            'type'=>$c->captype,
            'field'=>'cap_'.md5($c->name),
